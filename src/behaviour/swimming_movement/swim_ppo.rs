...
impl BrainPoolSwim {
    ...
    pub fn apply_step<S: Component + Copy + SwimSlot>(
        ...
        // Ensure every species net exists BEFORE borrowing them immutably.
        for key in groups.keys().copied().collect::<Vec<u32>>() {
            self.ensure_species(key);
        }

        // Lever #3 (one readback per pool): issue ALL per-species forwards
        // first (they queue + run back-to-back on the GPU), then read every
        // output back in ONE sync via a single concatenated tensor — instead
        // of a blocking `into_data()` between each species. Lever #1: forwards
        // run on the inner no-grad backend (`valid()`). Deterministic species
        // order keeps the cat/scatter offsets aligned.
        ...
        let mu_all = Tensor::cat(actor_outs,  0).into_data().into_vec::<f32>().expect("swim actor forward");
        let v_all  = Tensor::cat(critic_outs, 0).into_data().into_vec::<f32>().expect("swim critic forward");
