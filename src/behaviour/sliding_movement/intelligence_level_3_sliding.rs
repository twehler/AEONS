// Intelligence Level 3 — Predator sliding REINFORCE pool.
//
...
//   * `IntelligenceLevel::Level3` enrolment filter.
//   * `HIDDEN = 32`.
//   * Prey type: `Hetero` if carnivore else `Photo`.
//
// The public symbols other modules import (`BrainPoolL3`, `BrainSlotL3`,
// `assign_brains_l3`, `free_brains_l3`, `apply_intelligence_level_3`) keep their
...
impl SlidingSlot for BrainSlotL3 {
...
pub struct BrainPoolL3(pub BrainPoolSliding);

impl Deref for BrainPoolL3 {
    type Target = BrainPoolSliding;
    fn deref(&self) -> &Self::Target { &self.0 }
}
impl DerefMut for BrainPoolL3 {
...
pub fn assign_brains_l3(
    ...
    for (e, organism, _inheritance, restore) in new.iter() {
        // Strict per-level dispatch: Level3 heterotrophs only.
...
pub fn apply_intelligence_level_3(
    ...
    mut pool:    NonSendMut<BrainPoolL3>,
    heteros:     Query<(Entity, &mut Organism, &Transform, &BrainSlotL3, Option<&Carnivore>), With<Heterotroph>>,
