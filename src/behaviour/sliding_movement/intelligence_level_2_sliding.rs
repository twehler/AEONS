// Intelligence Level 2 — Predator REINFORCE pool (Monte-Carlo
...
// pool as `Vec<f32>` indexed by slot (see `BrainPoolL2` below):
...
/// system (real `species_id`s start at 1). All unclassified L2 heterotrophs
...
pub struct SpeciesBrain {
    pub model:    PoolMlpL2<MyBackend>,
    pub opt:      Box<dyn BrainOptL2>,
...
pub fn assign_brains_l2(
    ...
    for (e, organism, _inheritance, restore) in new.iter() {
        // Enrol only Level2 heterotrophs — strict per-level dispatch. A wrong
        ...
        // limb-based L2 pool instead (see `behaviour/limb_based_movement/`).
        ...
        if let Some(r) = restore {
            ...
            match pool.restore_species(key, r) {
                ...
                Err(err) => error!("L2 hetero brain restore failed for {e:?}: {err} — using fresh slot"),
            }
        }
...
pub fn apply_intelligence_level_2(
    ...
    mut pool:    NonSendMut<BrainPoolL2>,
    mut heteros: Query<(Entity, &mut Organism, &Transform, &BrainSlotL2, Option<&Carnivore>), With<Heterotroph>>,
