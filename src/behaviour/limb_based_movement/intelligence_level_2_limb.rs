...
pub struct BrainSlotL2Limb(pub u32);

impl LimbSlot for BrainSlotL2Limb {
    fn slot(&self) -> u32 { self.0 }
}


pub struct BrainPoolL2Limb(pub BrainPoolLimb);

impl FromWorld for BrainPoolL2Limb {
...
pub fn free_brains_l2_limb(
    mut pool:    NonSendMut<BrainPoolL2Limb>,
    mut removed: RemovedComponents<BrainSlotL2Limb>,
) {
...
pub fn apply_intelligence_level_2_limb(
    mut pool: NonSendMut<BrainPoolL2Limb>,
    organisms: Query<(Entity, &mut Organism, &BrainSlotL2Limb)>,
    body_parts: Query<(
