...
impl BrainPoolLimb {
    ...
    pub fn apply_step<'w, 's, S: Component + Copy + LimbSlot>(
        ...
        // Ensure every species net exists BEFORE we borrow them immutably for
        // the forward pass (ensure_species needs &mut self).
        for key in groups.keys().copied().collect::<Vec<u32>>() {
            self.ensure_species(key);
        }

        // Lever #3 (one readback per pool): issue ALL per-species forwards
        // first — they queue on the GPU stream and execute back-to-back — then
        // read every output back in ONE sync via a single concatenated tensor,
        // instead of a blocking `into_data()` between each species (which
        // ping-ponged CPU↔GPU and stalled both). Lever #1: forwards run on the
        // inner no-grad backend (`valid()`); `train` still uses Autodiff.
        // A deterministic species order keeps the cat/scatter offsets aligned.
        ...
        let mut critic_outs: Vec<Tensor<crate::rl_helpers::InferBackend, 2>> = Vec::with_capacity(order.len());
        ...
        for (key, slots) in &order {
            ...
            // bounded set of shapes; padded rows are zero and sliced off below.
            ...
            actor_outs.push(brain.actor.valid().forward(obs_t.clone()));   // [pad, OUT]
            critic_outs.push(brain.critic.valid().forward(obs_t));         // [pad, 1]
        }
        // ONE readback for all actor rows, ONE for all critic rows.
        let mu_all = Tensor::cat(actor_outs,  0).into_data().into_vec::<f32>().expect("limb actor forward");
        let v_all  = Tensor::cat(critic_outs, 0).into_data().into_vec::<f32>().expect("limb critic forward");
        // Scatter back: concatenated rows follow `order`, each species
        // contributing `pad` rows (only the first `cnt` = slots.len() are real).
        let mut row_off = 0usize;
        for ((_key, slots), &pad) in order.iter().zip(pads.iter()) {
            for (i, &s) in slots.iter().enumerate() {
                let r = row_off + i;
                mu_buf[s * OUT..s * OUT + OUT].copy_from_slice(&mu_all[r * OUT..r * OUT + OUT]);
                v_buf[s] = v_all[r];
            }
            row_off += pad;
        }
