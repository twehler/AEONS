...
use crate::volumetric_growth::build_part_meshes;
        ...
        if is_swimmer {
            ...
            commands.entity(bp_entities[idx]).insert((
                ...
                AdditionalMassProperties::MassProperties(MassProperties {
                    ...
                    mass: 0.0,
                    // `.max(..._MIN)` is a numerical-stability floor: at very small
                    // GEOMETRY_SCALE the scale⁵ inertia underflows the multibody
                    // solver's conditioning → NaN. Clamp keeps the solve stable
                    // below scale 0.2 (bounds the runtime inertia_mult too).
                    principal_inertia: Vec3::splat(
                        (crate::simulation_settings::SWIM_LINK_INERTIA_FLOOR
                            * crate::rapier_setup::swim_tuning().inertia_mult)
                            .max(crate::simulation_settings::SWIM_LINK_INERTIA_FLOOR_MIN),
                    ),
...
fn spawn_loaded_organism(
    ...
    for (idx, bp) in body_parts_snapshot.iter().enumerate() {
        // Adult + smoothing on → smoothed mesh, else faceted. Opaque cells and
        // translucent cells (Gelly) build into separate meshes (see
        // `build_part_meshes`).
        let (opaque_mesh, translucent_mesh) = if bp.regrowable && !bp.ocg.is_empty() {
            build_part_meshes(&bp.ocg, adult && smoothing)
        } else {
            (None, None)
        };
        ...
        spawn_translucent_overlay(commands, meshes, &materials.body_blend, child, translucent_mesh);
...
fn spawn_colony(
    ...
    *spawned = true;
    ...
    commands.insert_resource(OrganismMaterials {
        ...
        body_blend: materials.body_blend.clone(),
    });
    let mut rng = rand::rng();
...
pub struct OrganismMaterials {
    ...
    /// Like `photo`/`hetero` (white, vertex-colour pass-through) but alpha-blended,
    /// so body parts containing translucent cells (e.g. Gelly) honour the
    /// per-vertex alpha. Opaque parts keep the plain `photo` material to avoid
    /// transparency-sorting artefacts.
    pub body_blend:  Handle<StandardMaterial>,
}
...
impl OrganismMaterials {
    pub fn new(materials: &mut Assets<StandardMaterial>) -> Self {
        ...
        Self {
            ...
            body_blend: materials.add(StandardMaterial {
                base_color: Color::WHITE,
    ...
    /// Material handle for one body part's OPAQUE mesh. Per-cell colour lives on
    /// the mesh (`Mesh::ATTRIBUTE_COLOR`), so this only chooses between the shared
    /// white body material and the debug-blue full-part override. Translucent
    /// cells render from a separate overlay mesh using `body_blend` — see
    /// `spawn_translucent_overlay`.
...
/// Attach a body part's translucent cells (e.g. Gelly) as an alpha-blended
/// child mesh of `part_entity`, with an identity local transform so it tracks
/// the part. Cosmetic only — no `BodyPartIndex`, collider, or gizmo. `mesh` is
/// the translucent sub-mesh from `build_part_meshes` (a no-op when `None`, the
/// common all-opaque case). Kept separate from the opaque mesh so the solid
/// cells stay in the depth-writing opaque pass.
fn spawn_translucent_overlay(
    commands:    &mut Commands,
    meshes:      &mut Assets<Mesh>,
    body_blend:  &Handle<StandardMaterial>,
    part_entity: Entity,
    mesh:        Option<Mesh>,
) {
    ...
    let overlay = commands.spawn((
        ...
        Visibility::Visible,
        Mesh3d(meshes.add(mesh)),
        MeshMaterial3d(body_blend.clone()),
        bevy::light::NotShadowCaster,
    )).id();
    commands.entity(part_entity).add_child(overlay);
}
...
pub fn spawn_organism(
    ...
    for (idx, bp) in body_parts.iter().enumerate() {
        ...
        // Opaque + translucent (Gelly) cells split into separate meshes.
        let (opaque_mesh, translucent_mesh) = if bp.regrowable && !bp.ocg.is_empty() {
            build_part_meshes(&bp.ocg, adult && smoothing)
        } else {
            (None, None)
