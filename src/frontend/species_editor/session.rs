...
///
/// For bilateral organisms, `ocg` holds the RIGHT-half cells only — the
/// left half is generated by `mirror_ocg_x` whenever a mesh refresh is
/// needed. The user can only place cells on the right half (x ≥
/// `MIN_X_BILATERAL`); the mirror appears automatically. This matches
/// the simulation's bilateral pipeline exactly.
#[derive(Resource, Default)]
pub struct SpeciesSession {
    pub draft: DraftSpecies,

    /// Right-half OCG (Bilateral) or full OCG (NoSymmetry). Indices
    /// are sequential 0..N.
    ...
    /// `true` once the user has clicked "Spawn first Cell". Locks the
    /// cyclers (they only affect the species before any cells exist)
    /// and reveals the bottom-panel + viewport-placement workflow.
...
impl SpeciesSession {
    ...
    /// Combined OCG for mesh rendering during editing: for Bilateral
    /// this returns right-half + mirrored-left as a single contiguous
    /// list with re-numbered sequential indices (the format
    /// `bilateral_body_part_from_right_ocg` and `build_mesh_from_ocg`
    /// expect). For NoSymmetry this is just `self.ocg` unchanged.
    pub fn mesh_ocg(&self) -> Vec<(usize, Vec3, CellType)> {
        match self.draft.symmetry {
            Symmetry::NoSymmetry => self.ocg.clone(),
            Symmetry::Bilateral => {
                let left = crate::body_part::mirror_ocg_x(&self.ocg);
                let mut combined: Vec<(usize, Vec3, CellType)> =
                    self.ocg.iter().chain(left.iter()).copied().collect();
                // Re-number sequentially so `build_mesh_from_ocg` sees a
                // consistent index ladder.
