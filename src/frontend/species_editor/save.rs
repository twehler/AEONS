...
pub fn encode_species_with_brain(
    ...
    buf.extend_from_slice(MAGIC_V11);
...
fn decode_species(bytes: &[u8]) -> std::io::Result<LoadedSpecies> {
    ...
    if !is_v11 && !is_v10 && !is_v9 && !is_v8 && !is_v7 && !is_v6 && !is_v5 && !is_v4 && !is_v3 && !is_v2 && !is_v1 {
        return Err(err("species magic mismatch (expected AEONSS01..AEONSS11)"));
    }
    ...
    let body_parts: Vec<LoadedBodyPart> = if is_v4 || is_v5 || is_v6 || is_v7 || is_v8 || is_v9 || is_v10 || is_v11 {
