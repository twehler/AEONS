    ...
    fn build(&self, app: &mut App) {
        app
            ...
            .add_systems(Update, (
                ...
                body_part_panel::handle_kind_button_clicks,
                body_part_panel::manage_kind_dropdown,
                body_part_panel::handle_kind_option_clicks,
                body_part_panel::handle_rename_input,
                body_part_panel::sync_body_part_rows,
            ))
            // Camera: Free flycam (shared) vs Orbit, selectable from the tool
            // panel. `snap` frames on entry; `sync` runs first so the orbit
            // switch reads the framed pose.
            .add_systems(Update, (
                (camera::snap_camera_on_mode_entry, camera::sync_species_camera).chain(),
                camera::orbit_camera_input,
                camera::handle_camera_mode_button,
            ))
            // Placement.
            .add_systems(Update, (
            ...
            // Editor-mode dropdown (Addition / Deletion / Diagnostics) +
            // Diagnostics hover/label.
            .add_systems(Update, (
                editor_mode::handle_editor_mode_clicks,
                editor_mode::sync_editor_mode_widget,
                editor_mode::update_diagnostics_hover,
                wrap::handle_wrap_click,
            ))
            // Deletion Mode: hover highlight, click-delete. Hover runs before
            // click so the click reads a fresh target.
            .add_systems(Update, (
                deletion::update_deletion_hover,
                deletion::handle_deletion_click,
            ).chain())
...
pub fn spawn_overlay_panels(parent: &mut ChildSpawnerCommands, top_offset_px: f32) {
    ...
    editor_mode::spawn_tool_panel(parent, top_offset_px);
