...
/// top of the screen — used by the merged-mode (in-simulation) editor
/// to push panels below any mode-switcher bar. The creation panel
/// anchors to `bottom: 0` so `top_offset_px` is structurally inert
/// here; the parameter exists for parity with `tool_panel::spawn` /
/// `inventory_panel::spawn`.
pub fn spawn_with_offset(parent: &mut ChildSpawnerCommands, draft: DraftOrganism, _top_offset_px: f32) {
    parent.spawn((
        CreationPanel,
        Node {
