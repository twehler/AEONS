...
// v012: organisms are authored/stored at the new 0.5 geometry scale, so this
// writes the v012 magic — the loader then does NOT apply the pre-v012 ×0.5
// migration rescale to editor-saved colonies. Must stay in lockstep with
// `colony_save_load.rs::SAVE_MAGIC`.
...
fn write_organism(buf: &mut Vec<u8>, tpl: &OrganismTemplate) {
    ...
    for p in &parts {
        for (_, ct) in &p.cells {
            if ct.is_photo() { photo_cells += 1; } else { non_photo_cells += 1; }
        }
    }
