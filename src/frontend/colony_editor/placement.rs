...
fn handle_right_click(
    ...
    organisms_q:    Query<(Entity, &Organism), With<OrganismRoot>>,
    // Per-part pick: each body-part entity's world transform tracks the ACTUAL
    // rendered body (kinematic root, dynamic multibody swimmer segments, …), so a
    // union of per-part spheres deletes ANY morphology. The old single
    // root-centred sphere under-covered long/chained swimmers whose body sits off
    // the root.
    parts_q:        Query<(&ChildOf, &crate::cell::BodyPartIndex, &GlobalTransform)>,
) {
    ...
    // Per-part spheres: test each body-part entity at its ACTUAL world position
    // (which tracks the rendered body for any representation) with its own cell
    // extent. The union covers every morphology — the old single root-centred
    // sphere under-covered long/chained multibody swimmers. Parts are a flat
    // hierarchy directly under the root, so `ChildOf::parent()` IS the root.
    let pad = crate::cell::MESH_PADDING;
    for (child_of, bp_idx, part_gxf) in &parts_q {
        let root = child_of.parent();
        let Ok((_, org)) = organisms_q.get(root) else { continue };
        let Some(bp) = org.body_parts.get(bp_idx.0) else { continue };
        if !bp.is_alive() { continue; }
        let extent = bp.cells.iter()
            .map(|c| c.local_pos.length())
            .fold(0.0_f32, f32::max);
        // `.max(1.0)`: clickability floor (matches the prior root-sphere behaviour).
        let radius = (extent + pad).max(1.0);
        if let Some(t) = ray_hits_sphere(ray.origin, *dir, part_gxf.translation(), radius) {
            keep_closer(Hit::Organism { entity: root }, t);
        }
    }
    ...
    match best {
        ...
        Some((Hit::Organism { entity }, _)) => {
            // The per-part pick returns the organism ROOT. If it's an editor
            // template, remove it the template way (keeps `session.templates` +
            // undo consistent — otherwise the list would keep a dangling entity).
            // Otherwise it's a wild organism → plain recursive despawn (drops
            // body-part children + mesh; RemovedComponents observers fire; wild
            // organisms aren't in the editor's reversible model).
            if let Some(index) = session.templates.iter().position(|t| t.entity == entity) {
                let removed = session.templates.swap_remove(index);
                commands.entity(removed.entity).despawn();
                if session.active_id == Some(removed.id) {
                    session.active_id = None;
