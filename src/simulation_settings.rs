...
// PERF (2026-06-23): frame profiling pinned ~91% of the frame to the rapier
// solve (StepSimulation), which scales ~linearly with this count. A sweep
// (env `AEONS_SOLVER_ITERS`) measured phys/step 14.3→7.6→4.7→2.9 ms at 8→4→2→1
// iters; dropping 8→4 took the swimmer-snake cohort from ~19 to a vsync-capped
// 60 FPS and broke the FixedUpdate catch-up spiral, with no joint separation /
// body culling. 4 suffices for reduced-coordinate MULTIBODY swimmers/segments
// (which cannot separate by construction). Impulse-joint WALKER colonies (e.g.
// Runners) were the reason 8 was chosen (no-separation); if they separate at 4,
// raise per-run with `AEONS_SOLVER_ITERS=8` or give walker bodies
// `AdditionalSolverIterations` (per-regime fix). Default lowered 8 → 4.
pub const LIMB_SOLVER_ITERATIONS: usize = 4;
...
/// MASTER ORGANISM-GEOMETRY SCALE — one knob for overall organism size. Every
/// length-based geometry constant derives from it: `cell::CELL_SPACING`,
/// `volumetric_growth::{EDGE_LEN, MIN_CENTER_DIST}`, `body_part::MIN_X_BILATERAL`
/// (all ∝ scale), and `SWIM_LINK_INERTIA_FLOOR` (∝ scale⁵, since a solid link's
/// inertia ≈ mass·length²). `1.0` = the original (oversized) geometry; `0.5` =
/// the current half-scale. Pre-v12 `.colony`/`.species` files (stored at the
/// canonical 1.0) are migrated ×GEOMETRY_SCALE on load.
///
/// SCOPE: organism geometry ONLY. World constants (sensory radii, map, water)
/// are intentionally independent (organisms are meant to be small relative to
/// the world). The drag/motor force CAPS are left unscaled — safe while scaling
/// DOWN (they become over-provisioned, never clamp), but revisit them before
/// scaling UP. Changing this once files already exist needs the save migration
/// (version-gated loaders), so it is not yet a free repeated knob.
pub const GEOMETRY_SCALE: f32 = 0.1;
...
/// ~0.18 (at the original 1.0 geometry scale) — make the Featherstone
/// articulated-inertia solve ill-conditioned, so the 3-axis spherical motor
/// occasionally integrates to a NaN joint coordinate (solver panic). Adding this
/// isotropic inertia lifts the thin axis to a well-conditioned value, stabilising
/// the solve. Tune: lower if limbs feel sluggish, raise if a NaN still slips
/// through (the non-finite safety net is the hard backstop either way).
///
/// SCALES WITH GEOMETRY: a solid link's inertia goes as mass·length² ∝ scale⁵,
/// so this floor must track the geometry scale to keep the SAME relative
/// conditioning (a fixed floor would dominate the now-tiny links and make
/// swimmers sluggish). At the 0.5 organism-geometry scale (EDGE_LEN halved),
/// the natural inertia is `GEOMETRY_SCALE⁵` of the original, so the floor
/// derives as `GEOMETRY_SCALE⁵` and tracks the master scale automatically
/// (= 1/32 at scale 0.5). (`AEONS_SWIM_INERTIA_MULT` still scales it at runtime.)
pub const SWIM_LINK_INERTIA_FLOOR: f32 =
    GEOMETRY_SCALE * GEOMETRY_SCALE * GEOMETRY_SCALE * GEOMETRY_SCALE * GEOMETRY_SCALE;

/// Absolute lower bound on the inertia floor — a NUMERICAL-stability guard,
/// applied at the use site (`colony::insert_limb_physics`). The floor conditions
/// the Featherstone multibody solve; it is NOT a dynamics knob (the swim motor is
/// `AccelerationBased`, so a larger floor doesn't slow strokes — only passive /
/// turning response). At very small `GEOMETRY_SCALE` the `scale⁵` floor underflows
/// the solver's f32 conditioning → NaN joint coordinate → `step_simulation` panic
/// (`min/max = NaN` in the motor-constraint clamp). This clamps the effective
/// floor to `0.2⁵` — the value at scale 0.2, the smallest scale verified stable —
/// so scales below 0.2 keep a conditioned solve. Raise if NaNs still appear.
pub const SWIM_LINK_INERTIA_FLOOR_MIN: f32 = 0.000_32;
...
/// term, so a large wander speed (this was once `20.0`) made the drag dwarf
/// photosynthesis and starve an underwater phototroph in a tick or two — a mass
/// despawn that reproduction then refilled, churning the population and lagging
/// the sim. At ≤ this speed the drag stays well under per-cell production.
/// (Amoeba-scale organisms should drift slowly anyway.)
pub const PHOTO_WANDER_MAX_SPEED: f32 = 2.0;

/// Energy cost per unit of elevation gained (gravitational-PE analogue), charged on
/// the climb accumulated since the last energy tick.
...
// PERFORMANCE-TEST cohort: a few challenging predators (Snake) over a large
// `ball_plankton` prey floor. Heavier and more dynamic than the old easy
// cohort (the predators eat + reproduce, growing the heterotroph population),
// so the brain pools are exercised under real load — better signal for the
// `BRAINTICK` performance measurements. The constant NAMES are retained (the
// cohort is still three `spawn_species_cohort` slots); only their contents
// changed. The actual trophic type / movement mode comes from each `.species`
// file, not from the constant name.
//
// "Predator" slot — 4 × Snake (a heterotroph that hunts the plankton floor).
pub const SPAWN_SWIMMER_PATH:  &str  = "species/Snake.species";
pub const SPAWN_SWIMMER_COUNT: usize = 4;
// Third slot disabled (kept so a walker cohort can be re-enabled easily).
pub const SPAWN_STRIDER_PATH:  &str  = "species/Strider.species";
pub const SPAWN_STRIDER_COUNT: usize = 0;
// Prey-floor slot — 100 × ball_plankton. Spawned through `spawn_species_cohort`,
// whose `name` ("ball_plankton") makes `spawn_species_instance` apply the
// persistent `BallPlankton` marker, so these are excluded from the
